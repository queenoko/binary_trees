...
/**
 * r_insert_node - This node value instertion in a AVL....
...
avl_t *r_insert_node(avl_t **tree, avl_t *parent, avl_t **new, int nval)
{
	...
	if (bival > 1 && (*tree)->left->n > nval)
	{
		*tree = binary_tree_rotate_right(*tree);
	}
	else if (bival > 1 && (*tree)->left->n < nval)
	{
		(*tree)->left = binary_tree_rotate_left((*tree)->left);
		*tree = binary_tree_rotate_right(*tree);
	}
	else if (bival < -1 && (*tree)->right->n < nval)
	{
...
avl_t *avl_insert(avl_t **tree, int value)
{
	...
	r_insert_node(tree, *tree, &newInsert, value);
	return (newInsert);
}
